...
static void clear_s2(void) {
	...
	scr.display_buff[7] &= BIT(0); // "i" от "°F"
	scr.display_buff[8] &= ~(BIT(4) | BIT(5) | BIT(6) | BIT(7)); // "-" от "°F" + s3
}

static void clear_s3(void) {
	scr.display_buff[7] &= ~(BIT(0)); // "i" от "°F"
	scr.display_buff[8] &= ~(BIT(0) | BIT(1) | BIT(2) | BIT(3)); // "-" от "°F" + s2
...
static void show_s3_number_x10(s32 number, u8 atr){
	...
	if (number > 99995) {
		// "Hi"
		scr.display_buff[8] |= BIT(1) | BIT(2) | BIT(3);
		scr.display_buff[9] |= BIT(3) | BIT(5) | BIT(6);
	} else if (number < -9995) {
		// "Lo"
		scr.display_buff[8] |= BIT(1) | BIT(3);
		scr.display_buff[9] |= BIT(2) | BIT(3) | BIT(7);
		scr.display_buff[10] |= BIT(6) | BIT(7);
	} else {
