...
void tuya_zigbee_ota(void) {
	...
	flash_write_status(0, 0); // flash_unlock();
	// search for start firmware address 0x008000 or 0x009000 or 0x010000 or 0x020000 ?
...
int main(void) {
	...
	if(*(u32 *)(ZIGBEE_BOOT_OTA_FADDR + 8) == ID_BOOTABLE
		|| *(u32 *)(ZIGBEE_BOOT_OTA_FADDR + FLASH_SECTOR_SIZE + 8) == ID_BOOTABLE
		|| *(u32 *)(ZIGBEE_BOOT_OTA_FADDR + FLASH_SECTOR_SIZE + FLASH_SECTOR_SIZE + 8) == ID_BOOTABLE) {
